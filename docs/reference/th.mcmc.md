...
R2OpenBUGS package. Estimates are based on the stochastic volatility
model for Pound–Dollar exchange rate data presented in the appendix of
Meyer and Yu (2002). The MCMC was run for 1100 simulations, thinning to
keep every 10th iteration, and treating the first 100 simulations as
burn-in.
...
(`svpdx`) and the `my1.txt` BUGS file contained in this package; see the
example below.
...
``` r
# empty plot
plot(NULL, type = "n", xlim = c(1, 945),
     ylim = range(th.mcmc), ylab = "Theta")
...
# file.show("my1.txt")
#
# # run OpenBUGS (include theta as a parameter or nothing will be plotted)
# my1.mcmc <- bugs(
#   data = list(n = length(svpdx$pdx), y = svpdx$pdx),
